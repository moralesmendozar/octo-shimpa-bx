...
#PARA GENERAR WORDCLOUDS
#Funcion para obtener wordcloud
def wcloud(docs,n,svpath):
    ...
    mywords=["del","de","la",'el','los','las','un','una','unos','por','en','que','lo','en','su','con','para','no','mas','grafic',
            'al','ante','par','agno','miembr','junta','algun','este','estos','esta','estas','ciento','cient','se','es','com','sobr',
            'sobre','asi','dich','tod','respect','baj','anterior','otro','banco','banxico','banxic','ha','entre','segun']
    for p in mywords:
             my_stop_words.add(p)

    
    #image = np.array(Image.open(d,path.join("bxlogo.png")))

    wordcloud = WordCloud(stopwords=my_stop_words,background_color='white', width=700,height=700).generate(docs)
    
    plt.imshow(wordcloud)
    plt.axis('off')
    #plt.show()
    plt.savefig(svpath+'\wordcloud'+ str(n)+'.png', dpi=200, bbox_inches='tight')
    
    #return g
#Para obtener wordclouds
path = "C:/Users/k14299/Documents/Python Dir/Comunicacion Banxico/Primera corrida/"
minutas=[33,34,35]
for n in minutas:
    analistas,docs=import_txt(n, path)
    docs = doStemm(docs)
    wcloud(docs,n,path)
    #i.set_canvas(plt.gcf().canvas)
