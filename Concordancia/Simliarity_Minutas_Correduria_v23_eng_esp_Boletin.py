...
#Número de minuta
def import_txt(n,path,lan):
    #lan es "" si espagnol, y "_eng" si Englisch    
    docs_collection=[]
    #analistas=["minuta"]
    pool = Pool(processes=4)    
    #proc = []  #Contains the processes to be run on parallel
    #Minuta
    pathin = path+"minuta"+ str(n) +lan+ ".pdf"
    ...
    # Boletin
    pathin = path+"boletin"+ str(n) + lan+".pdf"    
    ...
    pathins = [ path+"l_" + str(c)+"_" + str(n) + "_post minuta"+lan+".pdf" for c in corr]
    #print "pathins = ", pathins
#    print 'pathins = ', pathins   
    resultsTxt = [getTxt(pathein,lan) for pathein in pathins] #sin parallell
...
def doStemmOnDi(di,lan):
    termsi = tokenizacion(di)
    if(lan =="_eng"):
        termsii = [SnowballStemmer("english").stem(term) for term in termsi]
    else:
...
#    print '--------------------------------------------'
    ...
    for i in range(len(docs)):
        print i
        di = docs[i]
        dii = doStemmOnDi(di,lan)
        docs2.append(dii)
    return docs2

#Para obtener la Matriz tf-idf
def tfidf_mat(docs_collection, lan):
    ...
    #No sé si es correcto extender esta lista, pero no pasa nada, creo.
...
def todo(rangeOfMinutas,path,lan):
    #lan es "" si espagnol, y "_eng" si Englisch    
    graph_list=[]
    for x in rangeOfMinutas:#range(n0,n1+1):
        print "Minuta",x
        analistas,docs=import_txt(x, path,lan)
        
        docs = doStemm(docs,lan) #ya está el Stemming en getText  (en import_txt)
#==============================================================================
...
if __name__ == '__main__':
    path = "C:/Users/M15169/Documents/DerRuediger/comunicacion_Bx/CorreduriasYminutas/"
    #path = "H:\Pedidos\2016\161214 Comunicación Bancos Centrales\Concordancia\Primera corrida"
    tic = time.clock()
    #pool = Pool(processes = 4)
    #pool.map(todo, [33,34], path)
    #lang = ""
    lang = "_eng"
    todo([33,34,35], path,lang)
    #lang es "" si espagnol, y "_eng" si Englisch    
    #todo([33,34,35], path)
